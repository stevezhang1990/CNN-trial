...
# fold_img_name = test_img_name
# fold_annot = test_annot
# fp_result = open(param.db_dir + "result/ out.txt","a")

# for tid,img_name in enumerate(fold_img_name):
#     #"fold " + str(fid+1) + "/" + str(param.fold_num)
#     print "test: " + " img " + str(tid) + "/" + str(len(fold_img_name))

#     # img = Image.open(param.test_dir + img_name + ".jpg")
#     img = open_nc(img_name, timeind)

fp_result = open(param.db_dir + "result/ out.txt","a")
annot_fp = open('/home/the/scratch/test_db.txt', "r")
raw_data = annot_fp.readlines()

#pos image cropping
test_db = [0 for _ in xrange(len(raw_data))]

for tid,img_name in enumerate(raw_data):
    
    parsed_line = line.split(',')
    print "test: " + " img " + str(tid) + "/" + str(len(test_db))
